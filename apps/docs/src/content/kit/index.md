...
`@implementjs/kit` is the fastest way to build an implement app. You write pages and layouts as files in `src/routes`, and kit turns them into a fully wired [router](/docs/router) with typed params, server-side rendering in dev, and a prerendered static site on build — or, with an [adapter](/kit/adapters), a server that renders and answers endpoints per request.

If you have used [SvelteKit](https://svelte.dev/docs/kit) this will feel very familiar, we borrowed the conventions on purpose.

Under the hood kit is just a Vite plugin. It scans your routes directory, generates the router for you, and serves everything through `@implementjs/vite`'s SSR dev server and prerenderer. There's no runtime of its own, what ships to the browser is the same [`@implementjs/router`](/docs/router) router you could have written by hand — kit brings it along and resolves it for you, so it is not a dependency you install or a version you keep in step.
