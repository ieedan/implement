...
description: An index.server.ts runs on the server and feeds the page its data.
section: Server data
focus: src/routes/blog/[slug]/index.server.ts
---
...
1. In `src/routes/blog/[slug]/index.server.ts`, look the post up in `POSTS` by `params.slug` and return it as `{ post }`. Return `null` when the slug isn't in `POSTS`, so unknown posts keep the not-found rendering.
