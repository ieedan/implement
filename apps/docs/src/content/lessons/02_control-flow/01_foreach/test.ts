...
export default async function test() {
	...
	for (const title of ["Read the docs", "Write a component", "Ship it"]) {
		expect(screen.container, `The list should show the "${title}" todo's title.`).toHaveTextContent(
			title,
		);
	}
	...
	expect(screen.container, "The new todo's title should appear in the list.").toHaveTextContent(
		"Learn ForEach",
	);
}
