...
It is also careful about _which_ subscription it matches. `watch` and the standalone `subscribe` run the effect immediately with the current values, which is what `ImplementEffect` does. A readable's own `subscribe` and `onChange` methods deliberately **skip** that first run — swapping either of those for an effect would change what your code does, so the rule leaves them alone:

```ts
// not reported — onChange skips the current value, Watch would not
