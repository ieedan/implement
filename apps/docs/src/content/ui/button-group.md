...
Copy the file below to `src/lib/components/ui/button-group.ts`. It imports [`separator`](/ui/separator) from the same directory, so install that too. Then, on top of `@implementjs/core` and `@implementjs/primitives`:
