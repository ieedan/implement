import { tutorials as generated, type Tutorial as GeneratedTutorial } from "../../.velite";
import { contentError } from "./content-error";
import { stripLessonSource } from "./lesson-source";

/**
 * The tutorial lessons, with their editable source files attached.
 *
 * Kept out of `./content` on purpose. Assembling a lesson calls into the raw
 * source globs below, and Rollup cannot prove those calls pure — so the array
 * is retained wherever this module is imported, and the lesson sources come
 * with it. While this lived in the content barrel every docs route paid for
 * the whole tutorial tree; now only `/tutorial/*` and the search index do.
 */
