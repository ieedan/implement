...
What `@packages/ui_v2` does not have yet, compiled while building the router
and `demos/tracker-v2` (a Linear-style issue tracker: routed list/detail
...
in [PAPER_CUTS.md](PAPER_CUTS.md).

Fixed since the v1 list, for the record: arbitrary attributes (`href`,
`disabled`, `aria-*`, `data-*` all work as typed props now — links and
accessible buttons are real), the router itself (typed params, typed
`Link`/`href`/`navigate`, persistent layouts, URL-synced search params),
element `this` bindings (`Div({ this: el })`), and `UIFramework.Document()` /
`Window()` (mount-scoped global listeners).
...
`UIFramework.Lifecycle({ onMount, onUnmount }, ...children)` hooks
...
patching resolved values in place (tracker-v2's comments do exactly this).
...
(PAPER_CUTS #6). A `query(() => promise)` returning
`{ data, error, loading, refetch }` as readables would delete most of
tracker-v2's comments plumbing.
...
- **Not-found within layouts** — the fallback replaces the whole tree
  (PAPER_CUTS #11); no catch-all segment.
...
clip, exactly like v1. Still the biggest visual-quality gap for real menus.
...
`UIFramework.Boundary(...children).Catch((error, reset) => ...)` isolates
...
No headless renderer, so none of tracker-v2 is tested except by clicking.
The router's matching/param logic is pure and would unit-test trivially if
mounting didn't require a real DOM.

## 11. Dev loop

Unchanged from v1: four processes (tsdown, tailwind, static server, API) and
full manual reloads. Every demo copies the same scaffold with only ports
changed — tracker-v2 copied it again, including generalizing the `serve`
invocation to `-s` for SPA fallback. A `create-app` template or dev-server
package would end the copying.
