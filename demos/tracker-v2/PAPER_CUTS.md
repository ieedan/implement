...
Sharp edges hit while implementing the `ui_v2` router and building
`demos/tracker-v2` on top of it. Missing features live in
[MISSING.md](MISSING.md); this file is about what exists but cut me.
...
The v1 tracker's "accept a plain value or a Readable" picker pattern needs
`isReadable` at the call site. It existed in `signal.ts` but wasn't in the
package index, so userland couldn't write maybe-reactive APIs at all.
...
network requests before settling. Tracker-v2's comments section had to use a
...
outlive it forever. `UIFramework.Lifecycle` now owns these: return the
...
"writable" is — the v1 papercut about the overload zoo, one layer deeper.
...
either narrow to static strings (what tracker-v2 does) or re-implement
subscription plumbing per prop. Same lesson as v1's papercut #7: userland
components need a "normalize a maybe-reactive prop" helper from the
framework.
...
Carried over from v1 verbatim: `watch(signals, fn)` runs `fn` at subscribe
time. The comments refetch logic had to use `onChange` instead — the
`{ immediate: false }` option (or doc note) still doesn't exist.
