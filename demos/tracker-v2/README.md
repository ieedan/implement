...
The Linear-style issue tracker, rebuilt on `@packages/ui_v2` to exercise the
new router end to end: typed route params as `Readable`s, typed
`Link`/`href`/`navigate`, a root layout that survives navigation, in-place
param patching, and URL-synced search.

Findings from building it live next to this file in
[MISSING.md](MISSING.md) and [PAPER_CUTS.md](PAPER_CUTS.md).

## Run

```sh
pnpm --filter @demos/tracker-v2 dev
```
...
The API is the v1 tracker server (Hono + zod-openapi over `node:sqlite`),
copied with only the port changed. Delete `server/tracker.db` to reseed.
...
- **Param patching**: prev/next in the detail header navigates
  ...
  `UIFramework.Lifecycle`-owned `id.onChange` reseeds the draft.
...
```
server/            v1 tracker API, port 4003
