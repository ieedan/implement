/**
 * Vendored from bits-ui's `computeCommandScore`, itself derived from
 * superhuman/command-score.
 *
 * @see https://github.com/huntabyte/bits-ui/blob/main/packages/bits-ui/src/lib/bits/command/compute-command-score.ts
...
// of fragments.
// NOTE: We score word jumps between spaces slightly higher than slashes,
...
/**
 * Scores how well `search` matches `command` (plus optional `keywords`),
 * from 1 (perfect match) down to 0 (no match).
 *
 * A continuous run of matched characters scores 1; matches at word starts
 * beat matches mid-word; transpositions and skipped characters are penalized.
 * This is Command's default filter.
 */
export function computeCommandScore(command: string, search: string, keywords?: string[]): number {
	command = keywords && keywords.length > 0 ? `${command} ${keywords.join(" ")}` : command;
